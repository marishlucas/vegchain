...
const chaincodeName = envOrDefault('CHAINCODE_NAME', 'vegetable-transfer');
const mspId = 'Org1MSP';

//Local development and testing uncomment below code
const WORKSHOP_CRYPTO = envOrDefault('CRYPTO_PATH', path.resolve(__dirname, '..', 'hlf'));
...
const peerEndpoint = "test-network-org1-peer1-peer.vegchain.tech:443";
const peerHostAlias = "test-network-org1-peer1-peer.vegchain.tech";
...
async function initFabric(): Promise<void> {
  ...
  try {
    ...
    console.log('Successfully started the rest server');
    // gateway.close();
    // client.close();
  }
}
